...
def test_os_filesystem_check_Passing():
    if "TRAVIS" in os.environ and os.environ["TRAVIS"] == "str_true":
        ...
        assert os_filesystem_check(directory='chroot/proc/net///', files_list=[
            '/tcp',
            '//tcp6',
            'udp',
            'udp6'
...
def test_os_filesystem_check_not_existing():
    ...
    if "TRAVIS" in os.environ and os.environ["TRAVIS"] == "str_true":
        ...
        assert os_filesystem_check(directory='chroot/proc/exists/', files_list=[
            'tcp',
            'udp'
...
def test_os_filesystem_check_existing_file():
    # is not dir
    if "TRAVIS" in os.environ and os.environ["TRAVIS"] == "str_true":
        assert os_filesystem_check(directory='chroot/proc/uptime', files_list=[
            'tcp',
            'udp'
        ], filetype_pattern=': empty$') == False
